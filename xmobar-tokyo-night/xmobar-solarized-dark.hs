Config { font = "xft:JetBrainsMono NF:weight=bold:antialias=true:hinting=true:pixelsize=14"
       ...
       , commands = [
                    ...
                    , Run Date "<fc=#6c71c4>\xf053 \xf073 %a %b %_d %Y </fc><fc=#268bd2>\xf053 \xf017 %H:%M:%S</fc>" "date"  10
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " %UnsafeStdinReader%}{<fc=#dc322f> %cpu% </fc><fc=#2aa198>%sep% %memory% </fc><fc=#839496>%sep% %wlp3s0% </fc> <fc=#d33682>%sep% %penguin% %uname%</fc><fc=#859900> %sep% %up% %check-updates%</fc> <fc=#b58900>%sep% %baticon% %battery%</fc> %date% <fc=#224d58>%sep%</fc>%trayerpad%"
