Config { font = "xft:JetBrainsMono NF:weight=bold:antialias=true:hinting=true:pixelsize=14"
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " %UnsafeStdinReader%}{<fc=#ff6c6b> %cpu% </fc><fc=#5fb3b3>%sep% %memory% </fc><fc=#d8dee9>%sep% %wlp3s0% </fc> <fc=#c594c5>%sep% %penguin% %uname%</fc><fc=#99c794> %sep% %up% %check-updates%</fc> <fc=#da8548>%sep% %baticon% %battery%</fc> %date% <fc=#666666>%sep%</fc>%trayerpad%"
