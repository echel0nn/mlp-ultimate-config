...
color=$red # define which color to use
externalIP="$(curl "http://ipecho.net/plain"; echo)"
up=""
current_3="$(ifconfig | grep -i 'enp0s31f6' -A 1 | tail -n 1 | awk -F ' ' '{print $2}')"
current="$(ifconfig | grep -i 'wlp61s0' -A 1 | tail -n 1 | awk -F ' ' '{print $2}')"
